...
let mediciones = JSON.parse(

    localStorage.getItem("TCE_RESULTADOS")
...
const datosDimensiones = {
    ...
    bienestar: promedioBienestar,

    inclusion: promedioInclusion,

    buenTrato: promedioBuenTrato,

    participacion: promedioParticipacion,

    seguridad: promedioSeguridad,

    pertenencia: promedioPertenencia,

    relaciones: promedioRelaciones
...
function actualizarTermometro(promedio){
    ...
    const niveles = [
        ...
        document.getElementById("nivel"),
        document.getElementById("nivelEncuesta")
    ...
    const categorias = [
        ...
        document.getElementById("resultadoCategoria"),
        document.getElementById("resultadoCategoriaEncuesta")
    ...
    const promedios = [
        ...
        document.getElementById("promedioTermometro"),
        document.getElementById("promedioTermometroEncuesta")

    ];

    promedios.forEach(campo => {

        if(!campo) return;

        campo.innerText = promedio.toFixed(2);
    ...
    const interpretaciones = [
        ...
        document.getElementById("interpretacionEncuesta")

    ];

    interpretaciones.forEach(campo=>{

        if(!campo) return;

        campo.innerText = info.interpretacion;
    ...
    const recomendaciones = [
        ...
        document.getElementById("recomendacionEncuesta")

    ];

    recomendaciones.forEach(campo=>{

        if(!campo) return;

        campo.innerText = info.recomendacion;
...
function actualizarDiagnostico(datos, promedio, sufijo = ""){

    const diagnostico = analizarDimensiones(datos);

    const alerta = document.getElementById(
        "alertaDimensiones" + sufijo
    );
    ...
    if(alerta){
        ...
        if(diagnostico.criticas.length > 0){
            ...
            html += "<h4>⚠ Dimensiones Prioritarias</h4><br>";
            html += diagnostico.criticas.join("<br><br>");

        }

        if(diagnostico.fortalezas.length > 0){

            html += "<br><br><h4>✅ Fortalezas Detectadas</h4><br>";
            html += diagnostico.fortalezas.join("<br>");
        ...
        if(
            ...
            html = "Sin diagnóstico disponible.";
    ...
    const indice = document.getElementById(
        "indiceRiesgo" + sufijo
    );
...
function mostrarDimensiones(datos){
    ...
    const dimensiones = {
        ...
        resBienestarEncuesta: datos.bienestar,

        resInclusion: datos.inclusion,
        resInclusionEncuesta: datos.inclusion,

        resBuenTrato: datos.buenTrato,
        resBuenTratoEncuesta: datos.buenTrato,

        resParticipacion: datos.participacion,
        resParticipacionEncuesta: datos.participacion,

        resSeguridad: datos.seguridad,
        resSeguridadEncuesta: datos.seguridad,

        resPertenencia: datos.pertenencia,
        resPertenenciaEncuesta: datos.pertenencia,

        resRelaciones: datos.relaciones,
        resRelacionesEncuesta: datos.relaciones
    ...
    for(const id in dimensiones){
        ...
        celda.innerText = Number(valor).toFixed(2);
...
function actualizarVisualizaciones(datos, promedio){

    actualizarTermometro(promedio);

    mostrarDimensiones(datos);

    actualizarDiagnostico(datos, promedio);

    actualizarDiagnostico(datos, promedio, "Encuesta");
...
const crearEncuestaBtn =
document.getElementById("crearEncuestaBtn");
...
crearEncuestaBtn.addEventListener("click", async () => {
    ...
    const responsable =
    document.getElementById("responsableEncuesta").value;

    const participantes =
    document.getElementById("cantidadEncuesta").value;
    ...
    const encuestasActivas = JSON.parse(
        localStorage.getItem("ENCUESTAS_ACTIVAS")
    ) || {};

    encuestasActivas[codigo] = {
    ...
    localStorage.setItem(
        ...
        JSON.stringify(encuestasActivas)
    ...
    resultadoEncuesta.innerHTML = `
        ...
        <p><strong>Participantes esperados:</strong> ${participantes}</p>
    ...
    document.getElementById("estadoEncuesta").innerHTML = `

        <h3>📊 Estado de la encuesta</h3>

        <p>
            <strong>Respuestas recibidas:</strong>
            <span id="contadorRespuestas">0</span>
        </p>

        <p>
            <strong>Participantes esperados:</strong>
            <span id="participantesEsperados">${participantes}</span>
        </p>

        <p>
            <strong>Avance:</strong>
            <span id="porcentajeEncuesta">0%</span>
        </p>

        <button id="procesarEncuestaBtn">

            📊 Generar resultados automáticos

        </button>

        <button id="cerrarEncuestaBtn">
...
document
    .getElementById("procesarEncuestaBtn")
    .addEventListener("click", procesarEncuesta);

document
    .getElementById("cerrarEncuestaBtn")
    .addEventListener("click", cerrarEncuesta);

document
    .getElementById("estadoEncuesta")
    .classList.remove("oculto");

    console.log(
    document.getElementById("estadoEncuesta")
);
        ...
        document
            .getElementById("contadorRespuestas")
            .textContent = 0;

        document
            .getElementById("participantesEsperados")
            .textContent = participantes;

        document
            .getElementById("porcentajeEncuesta")
            .textContent = "0%";
...
function cargarEncuestaEstudiante() {

    const parametros = new URLSearchParams(
        window.location.search
    );

    const codigoEncuesta = parametros.get(
        "encuesta"
    );

    // Si no existe código, salir
    ...
    // Ocultar interfaz del administrador
    ...
    document
        .getElementById("pantallaEncuesta")
        .classList.remove("oculto");
    ...
    const encuesta = encuestas[codigoEncuesta];
    ...
    // PRIORIDAD 1: encuesta cerrada

    if (encuesta.cerrada) {

        document.getElementById("pantallaEncuesta").innerHTML = `

            <div class="mensaje-final">

                <div class="mensaje-card mensaje-card-cerrada">
                    ...
                    <p>
                        ...
                        El proceso de respuesta ha concluido.
    ...
    // PRIORIDAD 2: estudiante ya respondió

    const yaRespondio = localStorage.getItem(

        `ENCUESTA_RESPONDIDA_${codigoEncuesta}`
            ...
            <div class="mensaje-final">
                ...
                <div class="mensaje-card">
                    ...
                    <p>
                        ...
                        Gracias por participar y contribuir a mejorar la convivencia escolar.
...
document.getElementById("datosEncuesta").innerHTML = `

<div class="datos-encuesta">
    ...
    <div class="dato-card">
        ...
        <div>
            ...
            <strong>${encuesta.responsable}</strong>
...
cargarEncuestaEstudiante();

actualizarBotonEncuesta();

function actualizarBotonEncuesta() {

    const filas = document.querySelectorAll("#tablaEncuestaEstudiante tbody tr");

    let completas = true;

    filas.forEach(fila => {

        if (!fila.querySelector('input[type="radio"]:checked')) {
            completas = false;
        }

    });

    const boton = document.getElementById("enviarEncuestaBtn");
...
document.querySelectorAll('#tablaEncuestaEstudiante input[type="radio"]').forEach(radio => {

    radio.addEventListener("change", actualizarBotonEncuesta);

});

document
    .getElementById("enviarEncuestaBtn")
    ?.addEventListener("click", enviarEncuestaEstudiante);

function enviarEncuestaEstudiante() {

    const respuestas = {};

    const filas = document.querySelectorAll(
        "#tablaEncuestaEstudiante tbody tr"
    );
    ...
    filas.forEach(fila => {
        ...
        const dimension = fila.dataset.dimension;

        const seleccionada = fila.querySelector(
    ...
    const codigoEncuesta = new URLSearchParams(
        window.location.search
    ).get("encuesta");
    ...
    if (!respuestasGuardadas[codigoEncuesta]) {

        respuestasGuardadas[codigoEncuesta] = [];
    }

    respuestasGuardadas[codigoEncuesta].push({
...
function actualizarEstadoEncuesta() {

    let codigo = new URLSearchParams(
        window.location.search
    ).get("encuesta");

    if (!codigo) {

        codigo = localStorage.getItem(
            "ENCUESTA_ACTUAL"
        );
    ...
    const respuestas = JSON.parse(
        localStorage.getItem("RESPUESTAS_ESTUDIANTES")
    ) || {};

    const encuesta = encuestas[codigo];

    if (!encuesta) return;

    const cantidadRespuestas =
        respuestas[codigo]?.length || 0;

    const participantes =
        Number(encuesta.participantes);

    const porcentaje = Math.round(

        (cantidadRespuestas / participantes) * 100

    );

    document
        .getElementById("contadorRespuestas")
        .textContent = cantidadRespuestas;
...
function procesarEncuesta() {
    ...
    const codigo = localStorage.getItem(
        "ENCUESTA_ACTUAL"
    );
    ...
    const respuestas = respuestasGuardadas[codigo];
    ...
    respuestas.forEach(estudiante => {

        sumaBienestar += estudiante.respuestas.bienestar;

        sumaInclusion += estudiante.respuestas.inclusion;

        sumaBuenTrato += estudiante.respuestas.buenTrato;

        sumaParticipacion += estudiante.respuestas.participacion;

        sumaSeguridad += estudiante.respuestas.seguridad;

        sumaPertenencia += estudiante.respuestas.pertenencia;

        sumaRelaciones += estudiante.respuestas.relaciones;

    });

    const total = respuestas.length;

    const promedioBienestar = sumaBienestar / total;
    const promedioInclusion = sumaInclusion / total;
    const promedioBuenTrato = sumaBuenTrato / total;
    const promedioParticipacion = sumaParticipacion / total;
    const promedioSeguridad = sumaSeguridad / total;
    const promedioPertenencia = sumaPertenencia / total;
    const promedioRelaciones = sumaRelaciones / total;
    ...
    const categoria = obtenerCategoria(promedioGeneral);
    ...
    const nuevaMedicion = {
        ...
        dimensiones: {
            ...
            bienestar: promedioBienestar.toFixed(2),

            inclusion: promedioInclusion.toFixed(2),

            buenTrato: promedioBuenTrato.toFixed(2),

            participacion: promedioParticipacion.toFixed(2),

            seguridad: promedioSeguridad.toFixed(2),

            pertenencia: promedioPertenencia.toFixed(2),

            relaciones: promedioRelaciones.toFixed(2)
    ...
    alert("✅ Encuesta procesada correctamente.");
...
function cerrarEncuesta() {
    ...
    const confirmar = confirm(
        ...
        "¿Deseas cerrar la encuesta?"

    );

    if (!confirmar) return;

    encuestas[codigo].cerrada = true;
